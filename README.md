...
### 含まれるもの / 含まれないもの
- **含まれるもの**：ノートブック、スクリプト、DBスキーマ（`sql/`）、LaTeX原稿（`thesis/`）、依存関係（`requirements.txt`）。
...
docker compose up -d初回起動時に `sql/001_schema.sql` によりテーブルが作成されます。
